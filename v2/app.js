...
const STORAGE_KEY = 'wizjonerka_demo_v1';
...
function applyTheme(t){ if(t==='dark'){ document.body.classList.add('dark'); themeToggle.textContent='☀️'; } else { document.body.classList.remove('dark'); themeToggle.textContent='🌙'; } }
applyTheme(state.meta.theme || 'light');
themeToggle.addEventListener('click', ()=>{ state.meta.theme = document.body.classList.contains('dark') ? 'light' : 'dark'; applyTheme(state.meta.theme); save(); });
